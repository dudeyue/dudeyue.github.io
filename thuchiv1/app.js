...
function seed(){
 ...
 return {version:3,budget:500000,wallets:[
...
function normalize(data){
 ...
 return {version:3,budget:Number(data.budget??data.monthlyBudget??data.nganSach??500000)||0,wallets:wallets.length?wallets:base.wallets,transactions:txs,settings:{hideBalance:!!(data.settings?.hideBalance)}};
}
...
function render(){
 ...
 const monthExpense=(()=>{const n=new Date();return state.transactions.filter(x=>x.type==='expense'&&new Date(x.date).getMonth()===n.getMonth()&&new Date(x.date).getFullYear()===n.getFullYear()).reduce((s,x)=>s+x.amount,0)})();
...
function init(){
 ...
 $('#toggleBalance').onclick=()=>{state.settings.hideBalance=!state.settings.hideBalance;save();render()};$('#openBudget').onclick=()=>{$('#budgetInput').value=new Intl.NumberFormat('vi-VN').format(state.budget);openSheet('#budgetSheet')};$('#saveBudget').onclick=()=>{state.budget=num($('#budgetInput').value);save();closeSheets();render();toast('Đã cập nhật ngân sách')};
