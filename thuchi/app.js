...
function seed(){
 ...
 return {version:5,budget:500000,categoryBudgets:{food:250000,fuel:150000,shopping:100000},budgetAlerts:[80,100],wallets:[
  {id:cashId,name:'Tiền mặt',balance:320000,icon:'cash'},
  {id:bankId,name:'MB Bank',balance:1680000,icon:'bank'}
 ],transactions:[],fixedCosts:[
  {id:uid(),name:'Phí nhóm ship',amount:450000,cycle:'monthly',dueDay:1,category:'bills',walletId:bankId,paidMonths:[]},
  {id:uid(),name:'Data',amount:70000,cycle:'monthly',dueDay:5,category:'bills',walletId:bankId,paidMonths:[]},
  {id:uid(),name:'Điện thoại',amount:100000,cycle:'monthly',dueDay:10,category:'bills',walletId:bankId,paidMonths:[]}
 ],debts:[],settings:{hideBalance:false}};
}
...
function normalize(data){
 ...
 return {version:5,budget:Number(data.budget??data.monthlyBudget??data.nganSach??500000)||0,categoryBudgets:data.categoryBudgets||{},budgetAlerts:data.budgetAlerts||[80,100],wallets:wallets.length?wallets:base.wallets,transactions:txs,fixedCosts:fixedCosts.length?fixedCosts:base.fixedCosts,debts,settings:{hideBalance:!!(data.settings?.hideBalance)}};
}
...
function renderAI(){
 ...
 const dueFixed=fixedForCurrentMonth().filter(x=>!x.paid&&x.dueDate-new Date()<4*86400000).length,dueDebt=state.debts.filter(x=>x.type==='payable'&&x.amount>x.paid&&new Date(x.dueDate)-new Date()<4*86400000).length;if(dueFixed||dueDebt){face='⏰';mood='Có nghĩa vụ sắp đến hạn';text=`${dueFixed?dueFixed+' khoản cố định':''}${dueFixed&&dueDebt?' và ':''}${dueDebt?dueDebt+' khoản nợ':''} cần xử lý trong vài ngày tới. Tiền khả dụng hiện là ${money(availableMoney())}.`;tags.unshift('Đã trừ tiền cam kết khỏi khả dụng')} $('#aiFace').textContent=face;$('#aiMood').textContent=mood;$('#aiInsight').textContent=text;$('#aiTags').innerHTML=tags.slice(0,3).map(x=>`<span>${x}</span>`).join('')
}
...
function fixedForCurrentMonth(){
 const key=monthKey(), now=new Date();
 return state.fixedCosts.map(x=>({...x,paid:(x.paidMonths||[]).includes(key),dueDate:new Date(now.getFullYear(),now.getMonth(),Math.min(28,Math.max(1,x.dueDay||1)))}))
}
function fixedRemainingThisMonth(){return fixedForCurrentMonth().filter(x=>!x.paid).reduce((s,x)=>s+x.amount,0)}
...
function dueLabel(date,paid=false){
 if(paid)return 'Đã thanh toán';
 const today=new Date();today.setHours(0,0,0,0);const d=new Date(date);d.setHours(0,0,0,0);const diff=Math.round((d-today)/86400000);
 if(diff<0)return `Quá hạn ${Math.abs(diff)} ngày`;if(diff===0)return 'Đến hạn hôm nay';if(diff===1)return 'Đến hạn ngày mai';return `Còn ${diff} ngày`
}
function renderMoneyLayers(){
 const assets=allAssets(),fixed=fixedRemainingThisMonth(),debt=debtRemaining('payable');
 $('#assetSnapshot').textContent=money(assets);$('#fixedSnapshot').textContent='−'+money(fixed);$('#debtSnapshot').textContent='−'+money(debt);
 const av=availableMoney(),el=$('#availableMoney');el.textContent=money(av);el.classList.toggle('negative',av<0)
}
function showFinancePanel(name){
 ui.financePanel=name;
 $$('#financeSwitch button').forEach(b=>b.classList.toggle('active',b.dataset.finance===name));
 $$('[data-finance-panel]').forEach(p=>p.classList.toggle('active',p.dataset.financePanel===name));
}
function renderFixed(){
 const items=fixedForCurrentMonth(),total=items.reduce((s,x)=>s+x.amount,0),paid=items.filter(x=>x.paid).reduce((s,x)=>s+x.amount,0);
 $('#fixedMonthTotal').textContent=money(total);$('#fixedPaidText').textContent=`Đã trả ${money(paid)}`;$('#fixedRemainingText').textContent=`Còn ${money(total-paid)}`;$('#fixedProgress').style.width=(total?paid/total*100:0)+'%';
 const list=$('#fixedList');
 if(!items.length){list.innerHTML='<div class="empty-state"><span>📅</span><strong>Chưa có khoản cố định</strong><small>Thêm phí nhóm, data, điện thoại hoặc các hóa đơn hàng tháng.</small></div>';return}
 list.innerHTML=items.sort((a,b)=>a.dueDay-b.dueDay).map(x=>{
  const overdue=!x.paid&&x.dueDate<new Date(new Date().setHours(0,0,0,0));
  return `<article class="obligation-item ${x.paid?'paid':''} ${overdue?'overdue':''}">
   <div class="obligation-top"><div class="obligation-icon">${x.paid?'✅':'📅'}</div><div class="obligation-copy"><strong>${x.name}</strong><span>Ngày ${x.dueDay} hàng tháng · ${wallet(x.walletId)?.name||'Chưa chọn ví'}</span></div><div class="obligation-amount"><strong>${money(x.amount)}</strong><small>${dueLabel(x.dueDate,x.paid)}</small></div></div>
   <div class="obligation-actions"><button data-edit-fixed="${x.id}">Chỉnh sửa</button>${x.paid?`<button data-unpay-fixed="${x.id}">Hoàn tác</button>`:`<button class="primary" data-pay-fixed="${x.id}">Đã thanh toán</button>`}</div>
  </article>`}).join('');
 $$('[data-edit-fixed]').forEach(b=>b.onclick=()=>openFixed(b.dataset.editFixed));
 $$('[data-pay-fixed]').forEach(b=>b.onclick=()=>payFixed(b.dataset.payFixed));
 $$('[data-unpay-fixed]').forEach(b=>b.onclick=()=>unpayFixed(b.dataset.unpayFixed));
}
function openFixed(id=null){
 ui.editingFixedId=id;const x=id?state.fixedCosts.find(v=>v.id===id):null;ui.fixedCycle=x?.cycle||'monthly';ui.fixedWalletId=x?.walletId||state.wallets[0]?.id;
 $('#fixedSheetTitle').textContent=id?'Sửa khoản cố định':'Thêm khoản cố định';$('#fixedNameInput').value=x?.name||'';$('#fixedAmountInput').value=x?new Intl.NumberFormat('vi-VN').format(x.amount):'';$('#fixedDueDayInput').value=x?.dueDay||1;$('#fixedCategoryInput').value=category(x?.category||'bills').name;
 $$('#fixedCycleChips .chip').forEach(b=>b.classList.toggle('active',b.dataset.cycle===ui.fixedCycle));$('#deleteFixed').classList.toggle('hidden',!id);renderFinanceWalletPicks();openSheet('#fixedSheet')
}
function saveFixed(){
 const name=$('#fixedNameInput').value.trim(),amount=num($('#fixedAmountInput').value);if(!name||!amount)return toast('Nhập tên và số tiền');
 const text=$('#fixedCategoryInput').value.toLowerCase();const cat=categories.find(c=>c.name.toLowerCase()===text)?.id||'bills';
 const old=ui.editingFixedId?state.fixedCosts.find(x=>x.id===ui.editingFixedId):null;
 const data={id:ui.editingFixedId||uid(),name,amount,cycle:ui.fixedCycle,dueDay:Math.min(28,Math.max(1,Number($('#fixedDueDayInput').value)||1)),category:cat,walletId:ui.fixedWalletId,paidMonths:old?.paidMonths||[]};
 if(ui.editingFixedId)state.fixedCosts[state.fixedCosts.findIndex(x=>x.id===ui.editingFixedId)]=data;else state.fixedCosts.push(data);save();closeSheets();render();toast('Đã lưu khoản cố định')
}
function payFixed(id){
 const x=state.fixedCosts.find(v=>v.id===id),key=monthKey();if(!x||x.paidMonths.includes(key))return;
 x.paidMonths.push(key);state.transactions.push({id:uid(),type:'expense',amount:x.amount,note:x.name,category:x.category||'bills',walletId:x.walletId,toWalletId:null,date:new Date().toISOString(),source:'fixed',sourceId:x.id,sourceMonth:key});save();render();toast('Đã ghi nhận thanh toán')
}
function unpayFixed(id){
 const x=state.fixedCosts.find(v=>v.id===id),key=monthKey();if(!x)return;x.paidMonths=x.paidMonths.filter(k=>k!==key);state.transactions=state.transactions.filter(t=>!(t.source==='fixed'&&t.sourceId===id&&t.sourceMonth===key));save();render();toast('Đã hoàn tác')
}
...
function init(){
 ...
 ['amountInput','budgetInput','walletBalanceInput','fixedAmountInput','debtAmountInput','debtPaidInput'].forEach(id=>$('#'+id).addEventListener('input',e=>{const n=num(e.target.value);e.target.value=n?new Intl.NumberFormat('vi-VN').format(n):''}));
